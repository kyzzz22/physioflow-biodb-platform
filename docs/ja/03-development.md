...
### PF 側：D8 ✅ 可視化（demo ブランチ、2026-08-29）
履歴マルチチャンネル曲線、リアルタイム窓、感情マップ、D7 特徴パネルを実装し、D3 の読戻しと D7 の分析結果を可視化する。BioDB 側は変更なし。詳細は [`13-d8-visualization.md`](13-d8-visualization.md)。

| 実装 | ファイル（PF demo ブランチ） | 説明 |
|---|---|---|
| 純幾何 | `src/analysis/chartGeometry.js` | min/max 間引き、座標変換と逆変換、nice 目盛り、パス/エリアパス、イベントマーカーのスナップ、感情座標 |
| マルチ曲線 | `src/analysis/MultiChannelChart.jsx` | 重ね描き、イベントマーカー、ホバー読み取り、ドラッグ拡大 |
| リアルタイム | `src/analysis/LiveChart.jsx` | スライドバッファ＋ rAF 間引き。データ源は呼び出し側注入（D9 完了後にデバイス直結） |
| 特徴パネル | `src/analysis/FeaturePanel.jsx` | D7 結果の図示（指標/帯域バー/HRV/EDA） |
| 感情マップ | `src/analysis/AffectMap.jsx` | 価値-覚醒サーカムプレックス、象限件数、軌跡線 |
| 接続 | `src/DataPanel.jsx` | 4 ビュー切替（単一/全チャンネル/特徴/感情マップ） |

**要点**：幾何計算を純関数に分離して Node で単体テスト可能に。Canvas ではなく SVG（パスをアサートできる）。min/max 間引きでピークのエンベロープを保持。欠損はゼロで描かず線を切る。検証：`node e2e-d8.mjs` 全 PASS——実際の読戻しデータで描画し、パスに NaN がないこと、マーカーが描画領域内にあること、期待した SVG 要素が出ることをアサート。単体テスト 19 件。

### WebUI プラットフォーム側（2026-08-30）
BioDB の nginx に全 UI を統合し、単一 origin（研究室 LAN）で運用できるようにした。詳細は [`14-webui-console.md`](14-webui-console.md)：
...
| WebUI ログイン | ✅ | Google OAuth + 共通認証状態 / API Client / 管理ページ保護 / 401 自動ログアウト（localhost で動作。LAN IP アクセスには HTTPS が必要） |
...
### 次のステップ計画
1. **短期（PF リポジトリ）**：D5 実機調整（Muse 実機が必要、コードは完成）→ D9 ストリーミングプッシュ（D8 リアルタイムの完全形はこれに依存）。BioDB 側依存は全て整備済みで、既存エンドポイントに直接接続可能。
2. **中期（PF リポジトリ）**：D9 ストリーミングプッシュ → D10 プラットフォームレベル権限/監査。
3. **WebUI 運用**：研究室 LAN での多端末利用に向け、HTTPS（自己署名）化とファイアウォール 5002 受信許可。Google ログインは localhost 以外では HTTPS 必須のため、HTTPS 導入後に LAN 端末からのログインが可能になる。
4. **長期**：実データ投入後に全経路（収集 → 分析 → 可視化 → 書き出し → 推送）を再検証。
5. **BioDB 側運用**：テスト残骸は整理済み（`exp_quality`、10:00 無タグ窓、単点 `exp_emotion`/`exp_cognition` 等を削除し、`exp_emotion_verify` と `evt_verify_001` は連携確認用に保持）；実データ接続後に結合エクスポートのメタデータと 48h 大窓性能を再検証。
...
| M2（Phase 3） | 脳波接続 + 分析 + 可視化 | 🚧 分析・可視化・WebUI 統合は完了。残るは「実デバイスが保存される」（D5 実機検証） |
| M3（Phase 4） | ストリーミング + 権限監査 | 未開発（D9 → D10） |
