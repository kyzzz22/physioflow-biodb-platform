...
### PF 侧：D8 ✅ 可视化（demo 分支，2026-08-29）
历史多列曲线、实时窗口、情感地图、D7 特征面板，消费 D3 读回与 D7 分析结果。BioDB 侧零改动。详见 [`13-d8-visualization.md`](13-d8-visualization.md)。

| 实现 | 文件（PF demo 分支） | 说明 |
|---|---|---|
| 纯几何 | `src/analysis/chartGeometry.js` | min/max 降采样、坐标映射与逆映射、nice 刻度、路径/面积路径、事件标记吸附、情感坐标 |
| 多列曲线 | `src/analysis/MultiChannelChart.jsx` | 多通道叠加、事件标记、悬停读数、拖拽缩放 |
| 实时模式 | `src/analysis/LiveChart.jsx` | 滑动缓冲 + rAF 节流；数据源由调用方注入（D9 完成后可直连设备） |
| 特征面板 | `src/analysis/FeaturePanel.jsx` | D7 结果图形化（指标/频带条/HRV/EDA） |
| 情感地图 | `src/analysis/AffectMap.jsx` | 效价-唤醒环格、象限计数、轨迹连线 |
| 接入 | `src/DataPanel.jsx` | 四视图切换（单通道/全部通道/特征/情感地图） |
